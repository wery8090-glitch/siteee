import { isCreatorEmail, CREATOR_UID } from "../shared/creators";
...
function mapProfile(profile: SupabaseProfile) { return { id: isCreatorEmail(profile.email) ? CREATOR_UID : Number(profile.id), openId: profile.open_id, username: profile.username, name: profile.name, email: profile.email, role: isCreatorEmail(profile.email) ? "developer" : profile.role ?? "user", status: profile.status ?? "active", createdAt: profile.created_at ? new Date(profile.created_at) : new Date(), updatedAt: profile.updated_at ? new Date(profile.updated_at) : new Date(), lastSignedIn: profile.last_signed_in ? new Date(profile.last_signed_in) : new Date(), lastLoginAt: profile.last_login_at ? new Date(profile.last_login_at) : null }; }
...
export async function updateSupabaseStatus(openId: string, status: "active" | "suspended" | "banned") { const rows = await rest<SupabaseProfile[]>(`profiles?open_id=eq.${encodeURIComponent(openId)}`, { method: "PATCH", headers: { Prefer: "return=representation" }, body: JSON.stringify({ status, updated_at: new Date().toISOString() }) }); return rows.length > 0; }
