...
import { createAutoReplyTemplate, deleteAutoReplyTemplate, directExtendSubscription, directIssueSubscription, directRedeemSubscriptionKey, directRevokeSubscription, directUpdateSubscription, getAutoReplySuggestion, getAutoReplyTemplates, maybeSendAutoReply, updateAutoReplyTemplate } from "./supabaseAutomation";
...
const fallbackPlans = [{ id: 1, slug: "free", name: "FREE" }, { id: 2, slug: "base", name: "BASE" }, { id: 3, slug: "premium", name: "PREMIUM" }, { id: 4, slug: "premium_beta", name: "PREMIUM + BETA" }, { id: 5, slug: "tester", name: "TESTER" }, { id: 6, slug: "media", name: "MEDIA" }];
async function resolvePlanForAdmin(planId: number) { try { const plans = await getSupabasePlans(); return plans.find(plan => plan.id === planId) ?? fallbackPlans.find(plan => plan.id === planId) ?? null; } catch { return fallbackPlans.find(plan => plan.id === planId) ?? null; } }
...
export const appRouter = router({
  ...
  support: router({
    ...
    autoReplySuggestion: staffProcedure.input(z.object({ ticketId: z.number().int().positive() })).query(({ input }) => getAutoReplySuggestion(input.ticketId)),
  ...
  admin: router({
    ...
    createSubscriptionKey: adminProcedure.input(z.object({ planId: z.number().int().positive(), maxActivations: z.number().int().min(1).max(100000), expiresAt: z.coerce.date(), prefix: z.string().trim().max(24).regex(/^[A-Za-z0-9-]*$/).optional(), customCode: z.string().trim().max(96).regex(/^[A-Za-z0-9-]+$/).optional() })).mutation(async ({ ctx, input }) => { if (input.expiresAt <= new Date()) throw new TRPCError({ code: "BAD_REQUEST", message: "Дата окончания должна быть в будущем." }); const plan = await resolvePlanForAdmin(input.planId); if (!plan || plan.slug === "free") throw new TRPCError({ code: "BAD_REQUEST", message: "Неизвестный тариф. Обновите список тарифов и примените миграции Supabase." }); const result = await callSupabaseSubscriptionApi(ctx.req, "admin_create_key", { plan_slug: plan.slug, max_activations: input.maxActivations, expires_at: input.expiresAt.toISOString(), prefix: input.prefix || "CHROMA", custom_code: input.customCode || undefined }) as { key: string; record: Record<string, unknown> }; await createAuditLog({ userId: ctx.user.id, action: "ADMIN_KEY_CREATED", metadata: { plan: plan.slug, maxActivations: input.maxActivations, custom: Boolean(input.customCode) } }); return result; }),
    issueSubscription: adminProcedure.input(subscriptionInput).mutation(async ({ ctx, input }) => { if (input.endsAt <= input.startsAt) throw new TRPCError({ code: "BAD_REQUEST", message: "End date must be after start date" }); let id: number | null = null; try { id = await directIssueSubscription(input); } catch { const result = await callSupabaseSubscriptionApi(ctx.req, "admin_issue_subscription", { user_id: input.userId, plan_id: input.planId, starts_at: input.startsAt.toISOString(), ends_at: input.endsAt.toISOString(), provider: input.provider, admin_note: input.adminNote ?? null }) as { id?: number }; id = result?.id ? Number(result.id) : null; } if (!id) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Не удалось записать подписку в Supabase" }); await createAuditLog({ userId: ctx.user.id, action: "ADMIN_SUBSCRIPTION_ISSUED", metadata: { subscriptionId: id, ...input, startsAt: input.startsAt.toISOString(), endsAt: input.endsAt.toISOString() } }); return { success: true, id } as const; }),
